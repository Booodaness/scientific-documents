...
\begin{definition}
A \textbf{proposition} is a formal expression which can take the forms \emph{true} ($T$) and \emph{false} ($F$).
\end{definition}
...
Informally\footnote{As we have not yet constructed numbers, any employment of such objects is for informal and intuitive purposes.}, if a logical operator $\mathcal{L}$ acts on $n$ propositions, there are $2^{n+1}$ ways of defining it. This corresponds to $2^n$ entries in the truth table for $\mathcal{L}$, each of which corresponds to some unique combination of entries whose output is fixed to have some truth value.
...
Unary operators are logical connectives which act on one proposition and produce another. Thus, there are $2^{1+1} = 4$ possible unary operators, listed below:
