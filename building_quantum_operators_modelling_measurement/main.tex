...
This reveals that a generalized form of the Born rule can be sufficiently characterized by measurement-invariant density operators. The analysis further leads naturally to the Many Worlds Interpretation (MWI) of quantum mechanics, although in an abstract sense of the concept of 'many worlds'.
\end{frame}
...
\item We now adopt the following notation for transition from an initial density (for a pure state at this stage) to a final density,
...
\begin{align*}
\pr_{\text{mixed}} \left( \left\{ \left\lvert \Phi_\alpha \right\rangle \right\}, \left\{ \pr \left( \left\lvert \Phi_\alpha \right\rangle \right) \right\} \right) : & = \sum_\alpha \pr \left( \left\lvert \Phi_\alpha \right\rangle \right) \pr \left( \widehat{\rho}; \left\lvert \Phi_\alpha \right\rangle \left\langle \Phi_\alpha \right\rvert \right) \\
& = \sum_\alpha \pr \left( \left\lvert \Phi_\alpha \right\rangle \right) \frac{\tr \left( \widehat{\rho} \left\lvert \Phi_\alpha \right\rangle \left\langle \Phi_\alpha \right\rvert \right)}{\tr \left( \left\lvert \Phi_\alpha \right\rangle \left\langle \Phi_\alpha \right\rvert \right)} \\
& = \tr \left[ \sum_\alpha \frac{\pr \left( \left\lvert \Phi_\alpha \right\rangle \right)}{\tr \left( \left\lvert \Phi_\alpha \right\rangle \left\langle \Phi_\alpha \right\rvert \right)} \widehat{\rho} \left\lvert \Phi_\alpha \right\rangle \left\langle \Phi_\alpha \right\rvert \right] \\
& = \tr \left[ \widehat{\rho} \sum_\alpha \frac{\pr \left( \left\lvert \Phi_\alpha \right\rangle \right)}{\tr \left( \left\lvert \Phi_\alpha \right\rangle \left\langle \Phi_\alpha \right\rvert \right)} \left\lvert \Phi_\alpha \right\rangle \left\langle \Phi_\alpha \right\rvert \right] \\
...
\begin{frame}{Measurement Axioms}
\begin{itemize}
\item Since going from transition of states to eigenstates, to transition of states to entire ensembles of superimposed states may seem like cheating, we will axiomatize the latter and recover the former.

\item It turns out that to do so, we need the following new axioms aside from the ones we stated in the beginning (i.e. all the traditional axioms for quantum mechanics except the Born rule). We will call these the 'measurement axioms' as it will be seen that the Born rule can be proved using them.
\end{itemize}
\end{frame}

\begin{frame}{}
\begin{block}{Measurement Axioms}
\begin{enumerate}
\item \textbf{Transition probability}: The probability of transition of a mixed state $\widehat{\rho}$ to a mixed state $\widehat{\rho^\prime}$ is given by,
...
$$\widehat{\rho} \to \widehat{\sigma} : = \sum_\alpha \sum_k \pr \left( \left\lvert \Psi_\alpha \right\rangle \right) \pr_\alpha \left( \left\lvert a_k \right\rangle \right) \left\lvert a_k \right\rangle \left\langle a_k \right\rvert$$

\item \textbf{Determinism}: A measurement is deterministic! I.e.,

$$\pr \left( \widehat{\rho}, \widehat{\sigma} \right) : = 1$$
\end{enumerate}
\end{block}

\begin{itemize}
\item We have already motivated transition probabilities; now, we will look at wavefunction collapse and determinism before deriving the Born rule!
\end{itemize}
\end{frame}
